    ...
    <div class="col-12">
        <div class="card mb-4">
			<div class="card-header pb-0">
				<div class="row">
					<div class="col">
						<h6>Tabla de Países</h6>
					</div>
					<div class="col text-end">
						<button type="button" @click="OPEN('POST')" class="btn btn-dark btn-sm mb-3">Crear País</button>
					</div>
				</div>
			</div>
			<div class="card-body px-0 pt-0 pb-2">
				<div class="table-responsive p-0">
					<table class="table align-items-center mb-0">
						<thead>
							<tr>
								<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 text-center">País</th>
							...
							<tr v-for="country in countries" :key="country.id">
								<td>
									<div class="px-3 text-center">
										<span class="text-secondary text-xs font-weight-bold">{{ country.name }}</span>
									</div>
								</td>
				...
				function() {
					...
					.then((resp) => {
						if(resp.data.records.length > 0) {
							_this.show = true
							_this.countries = resp.data.records
							_this.icon = 'success'
